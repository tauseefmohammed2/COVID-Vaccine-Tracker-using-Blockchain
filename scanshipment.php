...
<html lang="en">
  ...
  if (typeof ethereum !== 'undefined') {
    ...
    $('#form2').on('submit', function(event) {
        ...
        mapsLink = 'https://maps.google.com/?q=' + currentLat + ',' + currentLong;

        //var today = new Date();
        //var thisdate = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
        var info = "<span class='time-wrapper'><span class='time'>"+date+"</span></span></div><div class='desc locate'>Location : "+ prodlocation+"<br><a href="+mapsLink+" target='_blank'>Open in Google Maps</a></div>";
        ...
        web3.eth.getAccounts().then(async function(accounts) {
          ...
          .then(receipt => {
              var msg="Vaccine Details Updated ";
              $("#alertText").html(msg);
              $("#qrious").hide();
              $("#bottomText").hide();
              $(".customalert").show("fast","linear");

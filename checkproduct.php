      ...
      <center>
        <div class="centered">
          <form role="form" autocomplete="off">
              <input type="text" id="searchText" class="searchBox" placeholder="Enter Vaccine Shipment ID" onkeypress="isInputNumber(event)" required>
		...
		<i class='fa fa-qrcode'></i> Scan QR
          ...
          <!--<div style="width:75%;" class="card">-->
            <!--<div class="card-body">-->
            <div id="timeline-div" style="overflow-y:scroll; height:440px; width: 75%; display: none;"> 
              <div class="timeline-body"><ul id="timeline" class="timeline"></ul></div>
            </div>  
            <!--</div>-->
          <!--</div>--> 
          ...
          <a id='journey' class='formbtn' style="display:none;" href="" target="_blank">View Vaccine Journey</a>
    ...
    <script>
      ...
      $('form').on('submit', function(event) {
        ...
        contract.methods.searchProduct(greeting).call(function(err, result) {
          ...
          console.log(result);
          var locations = document.getElementsByClassName('locate');
          const p = [];
          for(var i=0; i<locations.length; i++){
            var string = locations[i].innerHTML;
            var selected = string.slice(11,22)+string.slice(23,33);
            p.push(selected);
          }
          var link = "https://www.google.com/maps/dir/?api=1&origin=";
          for(var i=0; i<p.length; i++){
            if(i == 0){
              link = link + p[i] + "&waypoints=";
              continue;
            }

            if(i == (p.length-1)){
              link = link + "&destination=" + p[i];
              continue;
            }
          ...
          link = link + "&travelmode=driving";
          console.log(link);
          $('#journey').attr("href", link);
